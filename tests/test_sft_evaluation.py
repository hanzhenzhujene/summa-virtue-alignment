...
def test_evaluate_predictions_does_not_leak_relation_type_from_prediction_metadata(
    tmp_path,
) -> None:
    base_config = build_fixture_dataset_config(
        tmp_path,
    ...
    dataset = build_dataset(config)
    serialize_built_dataset(dataset)
    predictions_path = tmp_path / "predictions_with_metadata_only.jsonl"

    example = dataset.split_result.split_examples["test"][0]
    with predictions_path.open("w", encoding="utf-8") as handle:
        handle.write(
            json.dumps(
                {
    ...
    assert metrics["overall"]["citation_exact_match"] == 1.0
    assert metrics["overall"]["relation_type_accuracy"] is None
