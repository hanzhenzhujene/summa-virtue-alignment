#!/usr/bin/env bash
# Generate and evaluate held-out base-model predictions for the smaller remote-GPU path.
set -euo pipefail
...
case "${MODE}" in
  test)
    ...
    INFERENCE_CONFIG="configs/inference/qwen3_0_6b_base_test.yaml"
    RUN_DIR="${ROOT_DIR}/runs/christian_virtue/qwen3_0_6b/base_test"
    SPLIT_NAME="test"
    ;;
  ood)
    ...
    DATASET_SENTINEL="${DATASET_DIR}/all.jsonl"
    INFERENCE_CONFIG="configs/inference/qwen3_0_6b_base_ood.yaml"
    RUN_DIR="${ROOT_DIR}/runs/christian_virtue/qwen3_0_6b/base_ood"
    SPLIT_NAME="ood_test"
    ;;
  *)
