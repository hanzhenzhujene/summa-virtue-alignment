...
from .justice_core_spec import (
    JUSTICE_CORE_EXTRA_CONCEPTS,
...
def build_justice_core_annotation_artifacts() -> dict[str, int]:
    """Build justice-core reviewed concept and annotation artifacts."""

    context = load_justice_core_context()
    ...
    write_jsonl(GOLD_DIR / "justice_core_reviewed_concepts.jsonl", context.concepts.values())
    write_jsonl(GOLD_DIR / "justice_core_reviewed_doctrinal_annotations.jsonl", doctrinal)
    write_jsonl(
...
def load_justice_core_context() -> JusticeCoreContext:
    """Load tract-specific structural records and the justice concept overlay."""
    ...
    for payload in JUSTICE_CORE_EXTRA_CONCEPTS:
...
def candidate_terms(context: JusticeCoreContext, concept_id: str) -> Iterable[str]:
...
def resolve_node_label_type(
    ...
    raise KeyError(f"Unknown justice-core node id: {node_id}")
