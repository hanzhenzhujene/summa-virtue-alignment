...
from .fortitude_closure_136_140_spec import (
    FORTITUDE_CLOSURE_136_140_EXTRA_CONCEPTS,
...
class FortitudeClosure136140Context:
...
def build_fortitude_closure_136_140_annotation_artifacts() -> dict[str, int]:
    """Build reviewed concept and annotation artifacts for II-II QQ. 136-140."""

    context = load_fortitude_closure_136_140_context()
    ...
    write_jsonl(
        GOLD_DIR / "fortitude_closure_136_140_reviewed_concepts.jsonl",
...
def load_fortitude_closure_136_140_context() -> FortitudeClosure136140Context:
    questions = {
    ...
    prior_reviewed_path = GOLD_DIR / "fortitude_parts_129_135_reviewed_concepts.jsonl"
    if prior_reviewed_path.exists():
        for payload in load_jsonl(prior_reviewed_path):
            concept_record = CorpusConceptRecord.model_validate(payload)
            registry[concept_record.concept_id] = concept_record
    for payload in FORTITUDE_CLOSURE_136_140_EXTRA_CONCEPTS:
        concept_record = CorpusConceptRecord.model_validate(payload)
        registry[concept_record.concept_id] = concept_record
    concepts = {
    ...
    return FortitudeClosure136140Context(
        questions=questions,
        articles=articles,
        passages=passages,
        concepts=concepts,
...
def build_reviewed_doctrinal_annotations(
    context: FortitudeClosure136140Context,
...
def resolve_node_label_type(
    ...
    raise KeyError(f"Unknown fortitude closure 136-140 node id: {node_id}")
