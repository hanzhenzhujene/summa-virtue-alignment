...
def build_connected_virtues_109_120_review_queue() -> dict[str, str]:
    """Build review queue summaries and one example packet for QQ. 109-120."""

    coverage = json.loads(
        (
            PROCESSED_DIR / "connected_virtues_109_120_coverage.json"
    ...
    queue_payload = {
        ...
        "subcluster_modeling_items": [
            record.proposal_id
            for record in candidate_relations
            if cluster_for_question(passages[record.source_passage_id].question_number)
            in {
                "self_presentation",
                "social_interaction",
                "external_goods",
    ...
    queue_path = PROCESSED_DIR / "connected_virtues_109_120_review_queue.json"
    ...
    packet_path = (
        PROCESSED_DIR
        / "review_packets"
        / f"{packet_question_id.replace('.', '_')}_connected_virtues_109_120_review_packet.md"
    )
...
def build_review_packet(
    *,
    ...
    lines = [
        f"# Connected Virtues 109-120 Review Packet: II-II q.{question.question_number}",
        "",
