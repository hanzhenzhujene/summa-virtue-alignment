...
from .theological_virtues_spec import (
    RELATION_SEEDS,
    THEOLOGICAL_VIRTUES_MAX_QUESTION,
    THEOLOGICAL_VIRTUES_MIN_QUESTION,
    TRACT_CONCEPT_IDS,
    TREATMENT_SEEDS,
    RelationSeed,
    TreatmentSeed,
...
class TheologicalVirtuesContext:
...
def build_theological_virtues_annotation_artifacts() -> dict[str, int]:
    """Build tract-specific reviewed theological-virtues annotation artifacts."""

    context = load_theological_virtues_context()
    ...
    write_jsonl(
        GOLD_DIR / "theological_virtues_reviewed_concepts.jsonl",
...
def load_theological_virtues_context() -> TheologicalVirtuesContext:
    """Load structural context plus tract-reviewed concept records."""
    ...
    questions = {
        ...
        if (
            ...
            and THEOLOGICAL_VIRTUES_MIN_QUESTION
    ...
    return TheologicalVirtuesContext(
        questions=questions,
        articles=articles,
        passages=passages,
        concepts=concepts,
...
def build_reviewed_doctrinal_annotations(
    context: TheologicalVirtuesContext,
) -> list[PilotAnnotationRecord]:
    """Build new reviewed doctrinal annotations for II-II questions 1-46."""
...
def build_reviewed_structural_editorial_annotations(
    ...
    """Build reviewed question-to-concept treatment correspondences for the tract."""
...
def candidate_terms(context: TheologicalVirtuesContext, concept_id: str) -> Iterable[str]:
...
def resolve_node_label_type(
    ...
    raise KeyError(f"Unknown theological virtues node id: {node_id}")
