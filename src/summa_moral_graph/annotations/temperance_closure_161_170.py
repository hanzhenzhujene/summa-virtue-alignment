...
from .temperance_closure_161_170_spec import (
    RELATION_SEEDS,
    TEMPERANCE_CLOSURE_161_170_EXTRA_CONCEPTS,
...
class TemperanceClosure161170Context:
...
def build_temperance_closure_161_170_annotation_artifacts() -> dict[str, int]:
    """Build reviewed concept and annotation artifacts for II-II QQ. 161-170."""

    context = load_temperance_closure_161_170_context()
    ...
    write_jsonl(
        GOLD_DIR / "temperance_closure_161_170_reviewed_concepts.jsonl",
...
def load_temperance_closure_161_170_context() -> TemperanceClosure161170Context:
    questions = {
    ...
    for payload in TEMPERANCE_CLOSURE_161_170_EXTRA_CONCEPTS:
        concept_record = CorpusConceptRecord.model_validate(payload)
        registry[concept_record.concept_id] = concept_record
    concepts = {
    ...
    return TemperanceClosure161170Context(
        questions=questions,
        articles=articles,
        passages=passages,
        concepts=concepts,
...
def build_reviewed_doctrinal_annotations(
    context: TemperanceClosure161170Context,
...
def resolve_node_label_type(
    ...
    raise KeyError(f"Unknown temperance closure 161-170 node id: {node_id}")
...
def extract_evidence_text(
    ...
    *,
    ...
    for hint in hint_list:
        ...
        if excerpt:
            return excerpt
    fallback = passage_text[:260].strip()
    if require_match and not fallback:
        raise ValueError(f"Could not match evidence hint in passage: {hint_list}")
    return fallback


def excerpt_around_hint(text: str, hint: str) -> str | None:
    index = text.casefold().find(hint.casefold())
    if index < 0:
        return None
    start = max(0, index - 80)
    end = min(len(text), index + len(hint) + 180)
