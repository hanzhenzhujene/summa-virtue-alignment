...
from .owed_relation_tract_spec import (
    OWED_RELATION_TRACT_EXTRA_CONCEPTS,
...
class OwedRelationTractContext:
...
def build_owed_relation_tract_annotation_artifacts() -> dict[str, int]:
    """Build owed-relation tract reviewed concept and annotation artifacts."""

    context = load_owed_relation_tract_context()
    ...
    write_jsonl(GOLD_DIR / "owed_relation_tract_reviewed_concepts.jsonl", context.concepts.values())
    write_jsonl(
...
def load_owed_relation_tract_context() -> OwedRelationTractContext:
    questions = {
    ...
    for payload in OWED_RELATION_TRACT_EXTRA_CONCEPTS:
    ...
    return OwedRelationTractContext(
        questions=questions,
        articles=articles,
        passages=passages,
        concepts=concepts,
...
def build_reviewed_doctrinal_annotations(
    context: OwedRelationTractContext,
...
def candidate_terms(context: OwedRelationTractContext, concept_id: str) -> Iterable[str]:
...
def resolve_node_label_type(
    ...
    raise KeyError(f"Unknown owed-relation tract node id: {node_id}")
