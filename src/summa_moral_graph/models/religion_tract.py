...
class ReligionTractBaseRecord(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)


class ReligionTractEdgeRecord(ReligionTractBaseRecord):
    edge_id: str
    subject_id: str
    ...
    def validate_support(self) -> "ReligionTractEdgeRecord":
...
class ReligionTractCoverageQuestionRecord(ReligionTractBaseRecord):
    question_id: str
    question_number: int = Field(ge=80, le=100)
    ...
    positive_act_usage: dict[str, int] = Field(default_factory=dict)
    excess_opposition_usage: dict[str, int] = Field(default_factory=dict)
    deficiency_opposition_usage: dict[str, int] = Field(default_factory=dict)
    sacred_object_usage: dict[str, int] = Field(default_factory=dict)


class ReligionTractValidationReport(ReligionTractBaseRecord):
