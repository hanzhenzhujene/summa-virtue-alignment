...
class TheologicalVirtuesBaseRecord(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)


class TheologicalVirtuesEdgeRecord(TheologicalVirtuesBaseRecord):
    edge_id: str
    subject_id: str
    ...
    def validate_support(self) -> "TheologicalVirtuesEdgeRecord":
...
class TheologicalVirtuesCoverageQuestionRecord(TheologicalVirtuesBaseRecord):
    question_id: str
    question_number: int = Field(ge=1, le=46)
...
class TheologicalVirtuesValidationReport(TheologicalVirtuesBaseRecord):
    ...
    @model_validator(mode="after")
    def validate_status(self) -> "TheologicalVirtuesValidationReport":
