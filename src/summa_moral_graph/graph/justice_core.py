...
from ..annotations.justice_core_spec import JUSTICE_CORE_MAX_QUESTION, JUSTICE_CORE_MIN_QUESTION
from ..models import (
    ArticleRecord,
    CorpusConceptRecord,
...
def build_justice_core_graph_artifacts() -> dict[str, int]:
    """Build justice-core node and edge exports."""
    ...
    write_jsonl(PROCESSED_DIR / "justice_core_structural_edges.jsonl", structural_edges)
    write_csv_rows(
        PROCESSED_DIR / "justice_core_nodes.csv",
        node_rows,
        [
            "node_id",
            "label",
    ...
    write_csv_rows(
        PROCESSED_DIR / "justice_core_reviewed_doctrinal_edges.csv",
    ...
    nx.write_graphml(
        doctrinal_graph,
        PROCESSED_DIR / "justice_core_reviewed_doctrinal.graphml",
    )
    nx.write_graphml(
        combined_graph,
        PROCESSED_DIR / "justice_core_reviewed_with_structural.graphml",
    )
    return {
        "nodes": len(node_rows),
...
def aggregate_edges(
    ...
    *,
    ...
    edges: list[JusticeCoreEdgeRecord] = []
...
def build_node_rows(
    ...
    for concept_id, concept_row in sorted(concepts.items()):
        rows.append(
            {
                "node_id": concept_id,
                "label": concept_row["canonical_label"],
                "node_type": concept_row["node_type"],
                "registry_status": concept_row["registry_status"],
                "description": concept_row["description"],
                "notes": " | ".join(cast(list[str], concept_row["notes"])),
            }
        )
        ...
        rows.append(
            {
                ...
                "description": "Structural question node for the justice core tract.",
                "notes": "",
            }
        )
    for article in sorted(articles.values(), key=lambda record: record.article_id):
        rows.append(
            {
                ...
                "node_type": "article",
                "registry_status": "structural",
                "description": "Structural article node for the justice core tract.",
                "notes": "",
            }
        )
    return rows
