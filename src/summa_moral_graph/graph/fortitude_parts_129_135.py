...
def build_fortitude_parts_129_135_graph_artifacts() -> dict[str, int]:
    """Build node and edge exports for the fortitude parts tract."""
    ...
    write_jsonl(
        PROCESSED_DIR / "fortitude_parts_129_135_structural_edges.jsonl",
        structural_edges,
    )
    write_csv_rows(
        PROCESSED_DIR / "fortitude_parts_129_135_nodes.csv",
        node_rows,
        [
            "node_id",
            "label",
    ...
    write_csv_rows(
        PROCESSED_DIR / "fortitude_parts_129_135_reviewed_doctrinal_edges.csv",
    ...
    nx.write_graphml(
        doctrinal_graph,
        PROCESSED_DIR / "fortitude_parts_129_135_reviewed_doctrinal.graphml",
    )
    nx.write_graphml(
        combined_graph,
        PROCESSED_DIR / "fortitude_parts_129_135_reviewed_with_structural.graphml",
    )
    return {
        "nodes": len(node_rows),
...
) -> list[FortitudeParts129135EdgeRecord]:
    grouped: dict[
        tuple[str, PilotRelationType, str, FortitudeParts129135Cluster],
        list[PilotAnnotationRecord],
    ] = defaultdict(list)
    for annotation in annotations:
        cluster = cluster_for_question(passages[annotation.source_passage_id].question_number)
        grouped[
            (
    ...
    for (subject_id, relation_type, object_id, cluster), records in sorted(grouped.items()):
        ...
        edges.append(
            FortitudeParts129135EdgeRecord(
                ...
                fortitude_parts_cluster=cluster,
            )
        )
    return edges
    ...
    for article in sorted(articles.values(), key=lambda record: record.article_id):
        ...
        edges.append(
            FortitudeParts129135EdgeRecord(
                ...
                fortitude_parts_cluster=cluster_for_question(question.question_number),
            )
        )
    return edges
        ...
        rows.append(
            {
                ...
                "node_type": "article",
                "registry_status": "structural",
                "description": "Structural article node for the fortitude parts 129-135 tract.",
                "notes": "",
            }
        )
...
def build_graph(
    ...
    edge_rows: Sequence[FortitudeParts129135EdgeRecord],
) -> nx.MultiDiGraph:
