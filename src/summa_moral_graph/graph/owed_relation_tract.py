...
def build_owed_relation_tract_graph_artifacts() -> dict[str, int]:
    """Build owed-relation tract node and edge exports."""
    ...
    write_jsonl(PROCESSED_DIR / "owed_relation_tract_structural_edges.jsonl", structural_edges)
    write_csv_rows(
        PROCESSED_DIR / "owed_relation_tract_nodes.csv",
        node_rows,
        [
            "node_id",
            "label",
    ...
    write_csv_rows(
        PROCESSED_DIR / "owed_relation_tract_reviewed_doctrinal_edges.csv",
    ...
    nx.write_graphml(
        doctrinal_graph,
        PROCESSED_DIR / "owed_relation_tract_reviewed_doctrinal.graphml",
    )
    nx.write_graphml(
        combined_graph,
        PROCESSED_DIR / "owed_relation_tract_reviewed_with_structural.graphml",
    )
    return {
        "nodes": len(node_rows),
...
def aggregate_edges(
    ...
    *,
...
) -> list[OwedRelationTractEdgeRecord]:
    ...
    edges: list[OwedRelationTractEdgeRecord] = []
    for (subject_id, relation_type, object_id), records in sorted(grouped.items()):
        first = records[0]
        due_modes = sorted(
            {
                due_mode
                for due_mode in (record.due_mode for record in records)
                if due_mode is not None
            }
        )
        if len(due_modes) > 1:
            raise ValueError(
                "Inconsistent due modes for owed-relation tract edge "
                f"{subject_id} {relation_type} {object_id}: {due_modes}"
            )
        edges.append(
            OwedRelationTractEdgeRecord(
                ...
                due_mode=due_modes[0] if due_modes else None,
        ...
        rows.append(
            {
                ...
                "node_type": "article",
                "registry_status": "structural",
                "description": "Structural article node for the owed-relation tract.",
                "notes": "",
            }
        )
...
def build_graph(
    ...
    edge_rows: Sequence[OwedRelationTractEdgeRecord],
) -> nx.MultiDiGraph:
