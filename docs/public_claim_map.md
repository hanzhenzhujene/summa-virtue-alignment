...
3. [docs/reports/christian_virtue_benchmark_improvements.md](./reports/christian_virtue_benchmark_improvements.md)
...
| The final full-corpus LoRA shows benchmark improvements beyond the headline held-out report. | The benchmark packet includes held-out Summa citation exact `0.0%` -> `71.2%`, Aquinas grounding score `37.7%` -> `74.2%`, VirtueBench V2 diagnostics, MMLU world religions `76.7%` -> `81.7%`, and Simplified-Chinese MMMLU ethics/philosophy transfer slices. | [docs/reports/christian_virtue_benchmark_improvements.md](./reports/christian_virtue_benchmark_improvements.md), [docs/reports/christian_virtue_benchmark_examples.md](./reports/christian_virtue_benchmark_examples.md), [README.md](../README.md) | `make report-christian-virtue-qwen2-5-1-5b-benchmark-packet` and `python scripts/build_christian_virtue_benchmark_improvements.py`; set `CHRISTIAN_VIRTUE_BENCHMARK_METRICS_ROOT` / `CHRISTIAN_VIRTUE_FINAL_ADAPTER_RUN_ROOT` if the final run lives in another worktree | The in-domain Aquinas rows are the lead claim; VirtueBench and MMLU/MMMLU rows are secondary diagnostic/transfer evidence. |
