...
| External MMLU world religions | accuracy | 60 | `76.7%` | `81.7%` | `+5.0 pp` | Secondary external evidence |
| External MMMLU-ZH business ethics | accuracy | 60 | `58.3%` | `61.7%` | `+3.3 pp` | Secondary external evidence |
| External MMMLU-ZH moral scenarios | accuracy | 60 | `25.0%` | `28.3%` | `+3.3 pp` | Secondary external evidence |
| External MMMLU-ZH philosophy | accuracy | 60 | `53.3%` | `55.0%` | `+1.7 pp` | Secondary external evidence |
| External MMLU moral scenarios | accuracy | 60 | `26.7%` | `28.3%` | `+1.7 pp` | Secondary external evidence |

## Interpretation

The full-corpus LoRA is clearly better at the thing it was trained for: Aquinas-grounded,
segment-cited doctrinal and relation answers. The table above pairs the untouched base model with
the full-corpus LoRA across the reported improvement surfaces.

External candidates were screened through a separate raw run/comparison layer. The public table
above includes the strongest external transfer slices: religion-adjacent MMLU world religions and
Simplified-Chinese MMMLU business ethics, moral scenarios, and philosophy.

## Next Recommendations

1. Lead with held-out exact citation and the Aquinas grounding probe.
2. Include VirtueBench and external rows with the A-position/external-transfer caveats attached.
